    ...
    if opt.t2cat:
        ...
        for r,d,t1,t2,t3 in zip(tra,tdec, T.tyc1[I2], T.tyc2[I2], T.tyc3[I2]):
            if not wcs.is_inside(r, d):
                continue
            ident = search_crossident(t1, t2, t3, d)
            if ident != None:
                annobjs.append((r, d, 'tycho2', [ident]))

    return annobjs


def search_crossident(t1, t2, t3, decl):
    tycho_id = 'TYC %i-%i-%i' % (t1,t2,t3)
    if (decl >= 0):
        # search in north catalog
        with open('cross_tyc2_north.csv') as csvfile:
            reader = csv.reader(csvfile, delimiter=',')
            for row in reader:
                if row[0] == tycho_id:
                    return row[1]
    else:
        # search in south catalog
        with open('cross_tyc2_south.csv') as csvfile:
            reader = csv.reader(csvfile, delimiter=',')
            for row in reader:
                if row[0] == tycho_id:
                    return row[1]
    # Option 1: return the original Tycho-2 identification
    # return tycho_id
    # Option 2: do not consider the identification
    return None
